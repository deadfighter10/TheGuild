...
export function SubmitLinkForm({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast("Failed to submit link", "error")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-5 rounded-xl border border-white/15 bg-void-900">
      ...
      <div>
        ...
        <input
          ...
          required
          placeholder="https://..."
          className="w-full px-4 py-2.5 bg-void-800 border border-white/15 rounded-lg text-white text-sm focus:outline-none focus:border-cyan-400/50 transition-colors placeholder:text-white/15 font-mono text-xs"
        />
      </div>
