...
function renderInline(text: string): string {
  return escapeHtml(text)
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-cyan-400/70 hover:text-cyan-400 underline underline-offset-2 transition-colors">$1</a>')
}
