...
describe("validateCreateThread", () => {
  it("allows a contributor to create a thread", () => {
    const result = validateCreateThread({ authorRep: 100, title: "Valid title", body: "Valid body" })
    expect(result).toEqual({ valid: true })
  })

  it("rejects observer (rep < 100)", () => {
    const result = validateCreateThread({ authorRep: 50, title: "Title", body: "Body" })
    expect(result.valid).toBe(false)
  })

  it("allows admin", () => {
    const result = validateCreateThread({ authorRep: -1, title: "Admin thread", body: "Admin body" })
    expect(result).toEqual({ valid: true })
  })

  it("rejects empty title", () => {
    const result = validateCreateThread({ authorRep: 100, title: "", body: "Body" })
    expect(result.valid).toBe(false)
  })

  it("rejects short title", () => {
    const result = validateCreateThread({ authorRep: 100, title: "Hi", body: "Body" })
    expect(result.valid).toBe(false)
  })

  it("rejects empty body", () => {
    const result = validateCreateThread({ authorRep: 100, title: "Valid title", body: "" })
    expect(result.valid).toBe(false)
  })
})
...
describe("validateCreateReply", () => {
  it("allows a contributor to reply", () => {
    const result = validateCreateReply({ authorRep: 100, body: "Good reply" })
    expect(result).toEqual({ valid: true })
  })

  it("rejects empty body", () => {
    const result = validateCreateReply({ authorRep: 100, body: "  " })
    expect(result.valid).toBe(false)
  })
})
...
describe("validateDeleteThread", () => {
  it("allows the author to delete their own thread", () => {
    const thread = makeThread({ authorId: "user-1" })
    const result = validateDeleteThread({ userId: "user-1", userRep: 100, thread })
    expect(result).toEqual({ valid: true })
  })

  it("allows a moderator to delete any thread", () => {
    const thread = makeThread({ authorId: "user-1" })
    const result = validateDeleteThread({ userId: "mod-user", userRep: 3000, thread })
    expect(result).toEqual({ valid: true })
  })

  it("allows admin to delete any thread", () => {
    const thread = makeThread({ authorId: "user-1" })
    const result = validateDeleteThread({ userId: "admin-user", userRep: -1, thread })
    expect(result).toEqual({ valid: true })
  })

  it("rejects non-author non-moderator", () => {
    const thread = makeThread({ authorId: "user-1" })
    const result = validateDeleteThread({ userId: "user-2", userRep: 500, thread })
    expect(result.valid).toBe(false)
  })
})
...
describe("validateDeleteReply", () => {
  it("allows the author to delete their own reply", () => {
    const reply = makeReply({ authorId: "user-2" })
    const result = validateDeleteReply({ userId: "user-2", userRep: 100, reply })
    expect(result).toEqual({ valid: true })
  })

  it("allows a moderator to delete any reply", () => {
    const reply = makeReply({ authorId: "user-2" })
    const result = validateDeleteReply({ userId: "mod-user", userRep: 3000, reply })
    expect(result).toEqual({ valid: true })
  })

  it("rejects non-author non-moderator", () => {
    const reply = makeReply({ authorId: "user-2" })
    const result = validateDeleteReply({ userId: "user-3", userRep: 500, reply })
    expect(result.valid).toBe(false)
  })
})
